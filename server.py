...
def fetch_yahoo_price(sym):
    ...
    try:
        import yfinance as yf  # noqa: PLC0415 — optional dependency, imported lazily
...
async def enrich_positions_from_db(positions):
    ...
    for p in positions:
        ...
        if (
            sec_type in ("STK", "ETF")
            and abs(p["market_price"] - avg_cost) < 0.001
            and avg_cost > 0
        ):
            needs_fix.add(sym)
...
async def pnl(horizon: str = "1d"):
    today = date.today()
    start = finance.horizon_start_date(horizon, today).isoformat()
    ...
    start_total_mv = sum(
        s.get("market_value", 0) for s in start_snap if s["symbol"] != "__REALIZED__"
    )
    ...
    for sym, c in sorted(curr_by_sym.items()):
        ...
        # If position opened during period, P&L is just unrealized from cost
        start_val = s.get("market_value", 0) if s else c.get("avg_cost", 0) * c.get("quantity", 0)
...
async def pnl_timeseries(from_date: str | None = None, to_date: str | None = None):
    if not from_date:
    ...
    by_date = finance.aggregate_snapshot_timeseries(snapshots)
    ...
    returns = finance.daily_linked_returns(
        dates, [by_date[d]["value"] for d in dates], [by_date[d]["pnl"] for d in dates]
    )
...
async def pnl_cumulative(period: str = "1y"):
    ...
    returns = [
        round((all_series[d]["value"] / all_series[d]["cost"] - 1) * 100, 2)
        if all_series[d]["cost"]
        else 0
        for d in dates
    ]
...
async def performance(
    benchmark: str = "SPY", from_date: str | None = None, to_date: str | None = None
):
...
def _exposure_breakdown(positions: list[dict], key: str) -> list[dict]:
    ...
    for p in positions:
        ...
        total += v
    return [
        {"category": k, "value": round(v, 2), "weight": round(v / total * 100, 2) if total else 0}
...
async def exposure_asset_class():
    # Asset class = IB's secType (STK/ETF/OPT/FUT/…). Historically this endpoint
    # was called "sector", which was a misnomer; /api/exposure/sector is kept as an
    # alias so existing frontend callers keep working.
    return _exposure_breakdown(await db.get_positions(), "sec_type")


@app.get("/api/exposure/sector")
async def exposure_sector():
    return await exposure_asset_class()


@app.get("/api/exposure/currency")
async def exposure_currency():
    return _exposure_breakdown(await db.get_positions(), "currency")
...
async def export_trades():
    ...
    output = io.StringIO()
    if trades:
        writer = csv.DictWriter(output, fieldnames=[k for k in trades[0] if k != "id"])
        writer.writeheader()
        for t in trades:
...
async def export_positions():
    ...
    output = io.StringIO()
    if positions:
        writer = csv.DictWriter(output, fieldnames=[k for k in positions[0] if k != "id"])
        writer.writeheader()
        for p in positions:
